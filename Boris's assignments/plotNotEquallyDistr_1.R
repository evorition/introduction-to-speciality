plotNotEquallyDistr_1 <- function(n=100){
  xBar <- c()
  temp <- c()
  
  for (value in seq(n)){
    set.seed(value)
    sample1 <- rnorm(n=1, mean=0, sd=value)
    temp <- c(temp, sample1)
  ...
  path <- file.path("/home", "/maxim/", "experiments_with_R", 
                    paste("not_equally_distr_1.png", sep=""))
  png(path, width=700, height=700, units="px")
  par(lwd=2)
  plot(xBar, type="l", col="red", main="Not Equally Disturbed (a)",
       ylim = c(-10, 10))
